...
SELECT  c.marca, c.modelo,
		SUM(v.distanciaPercorrida) AS distancia_total,
		e.nome
FROM Carro c
INNER JOIN Viagem v
ON v.idCarro = c.id
...
--3)Apresentar nome das empresas com a soma total da distância percorrida pelos carros
...
SELECT 	e.nome,
		MAX(v.distanciaPercorrida) AS maior_distancia
FROM Empresa e
INNER JOIN Carro c
ON c.idEmpresa = e.id
INNER JOIN Viagem v
...
SELECT 	e.nome,
		COUNT(c.idEmpresa) AS qtd_carros
FROM Empresa e
INNER JOIN Carro c
ON c.idEmpresa = e.id
GROUP BY e.nome
...
SELECT 	e.nome,
		...
		CASE WHEN (v.distanciaPercorrida > 1000)
			THEN 'R$ '+ CONVERT(VARCHAR(10), v.distanciaPercorrida * 15)
			ELSE 'R$ '+ CONVERT(VARCHAR(10), v.distanciaPercorrida * 10)
		END AS valor_total_ganho
FROM Empresa e
INNER JOIN Carro c
ON c.idEmpresa = e.id
INNER JOIN Viagem v
...
SELECT  c.marca, c.modelo,
		e.nome,
		CONVERT(VARCHAR(10),v.data,103) AS data_viagem
FROM Carro c
INNER JOIN Empresa e
ON c.idEmpresa = e.id
INNER JOIN Viagem v
...
SELECT 	c.marca, c.modelo,
		e.nome,
		DATEDIFF(DAY, v.data, '2023-01-01') AS qtd_dias_viagem
FROM Carro c
INNER JOIN Empresa e
ON c.idEmpresa = e.id
INNER JOIN Viagem v
...
WHERE v.data IN (
	SELECT v1.data
	FROM Viagem v1
	WHERE DATEDIFF(MONTH, v1.data, '2023-01-01') <= 3
)
ORDER BY qtd_dias_viagem, c.marca, c.modelo, e.nome
